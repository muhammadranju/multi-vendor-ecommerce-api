...
const generateAccessAndRefreshTokens = async (userId) => {
  try {
    ...
    // const accessToken = seller.generateToken();
    const accessToken = generateToken({
      email: seller.email,
      name: seller.name,
      role: seller.role,
      status: seller.status,
      TOKEN_SECRET: config.SELLER_ACCESS_TOKEN_SECRET,
      TOKEN_EXPIRY: config.SELLER_ACCESS_TOKEN_EXPIRY,
    });

    // Generate a refresh token for the seller
    const refreshToken = seller.generateToken();
